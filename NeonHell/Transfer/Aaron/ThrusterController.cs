	...
	void FixedUpdate(){
		...
		for (int i = 0; i < iThrusterCount; i++) {
			...
			if(Physics.Raycast(thrusters[i].position, -thrusters[i].up, out hit, alpha * fThrustDistance)){
				...
				fVPoint = fDThrusters[2*i + 1] - fDThrusters[2*i];
				...
				//fGForce += fVPoint < 0 ? -fVPoint : 0.0f;
				print ("G's: " + fGForce + " | distance: " + hit.distance + " | Desired distance: " + fThrustDistance + " | fVPoint: " + fVPoint + " | Added force: " + fAdditionalForce);
				rb.AddForceAtPosition(thrusters[i].up *(fThrustStrength * fGForce), thrusters[i].position);

				//if(fVPoint < 0)
				//	rb.AddForceAtPosition(thrusters[i].up 
			}
			else{
			}
		}//End for(int i = 0; i < iThrusterCount; i++)
